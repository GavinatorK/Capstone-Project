...
'''Function for Area Under Precision Recall Curve.'''

def prauc(observed, predicted):
    precision, recall, thresholds = precision_recall_curve(observed, predicted)
    return(auc(recall, precision))

'''Function to combine conditions and symptoms from Data, Sclist is symptomlabels. condlst is conditionlabels, filepath for each of the files'''
...
'''function returns probability for each condition from predictions'''
...
'''combining dataframes from 2006 and 2009'''

def combinedfs(symcond05, symcond09, condition05, condition09):
...
'''gridsearch for one vs rest classifier'''

def grdsrch(parameters, model, X_train, y_train):
...
'S3Q06','S3Q07','S3Q08','S3Q09','S3Q10','S3Q11','S3Q12','S3Q13','S3Q14']
    ...
    x2, y2=combinedfs(symcond05, symcond09, condition05, condition09)
    ...
    grdsrch(parameters, model, X_train, y_train)'''
    ...
    print sklearn.metrics.f1_score(y_test, predictions, average="samples")
